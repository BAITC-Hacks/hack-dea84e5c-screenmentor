...
Приложение открывается без логина и пароля после обновления контейнера. Для входа без NVIDIA-аккаунта также нужно включить публичный доступ Secure Link; его анонимная доступность пока не подтверждена. Стандартный Docker-запуск использует синтетический набор; исходные файлы кейса не включены в образ. Размещённая версия обновляется отдельно после git push — команды находятся в [инструкции Brev](DEPLOY_BREV.md#7-обновления-и-эксплуатация).
...
1. Открыть сайт без авторизации. Уточнить название набора: учебный пример нельзя выдавать за реальные данные кейса.
