...
**[Открыть ScreenMentor «Граф денег»](https://screenmentor-uwfohh6hz.gobrev.dev)** — приложение размещено в NVIDIA Brev. 23 сентября 2026 года владелец проекта подтвердил, что ссылка открывается. Это подтверждение доступа владельца; доступ отдельного аккаунта жюри необходимо проверить через Share у сервиса screenmentor. Ссылка на Jupyter не является ссылкой на продукт.

Логин приложения — `analyst`; пароль передаётся проверяющим отдельно и не хранится в репозитории. Secure Link может дополнительно требовать вход NVIDIA/Brev. Стандартный Docker-запуск использует синтетический набор; исходные файлы кейса не включены в образ. Размещённая версия обновляется отдельно после git push — команды находятся в [инструкции Brev](DEPLOY_BREV.md#7-обновления-и-эксплуатация).
...
1. Открыть сайт и войти. Уточнить название набора: учебный пример нельзя выдавать за реальные данные кейса.
...
Схема: исходные файлы → проверка данных → граф и признаки → роли, группы и приоритет → API → интерфейс и выгрузки. Её можно использовать как обязательную диаграмму решения на защите. [Описание архитектуры и ограничений](PROJECT_FEATURES.md#12-реализация-и-проверяемость).
