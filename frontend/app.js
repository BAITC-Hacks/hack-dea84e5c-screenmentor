...
async function activate(data) {
  ...
  $('role-filter').innerHTML = '<option value="">Все роли</option>' + Object.entries(summary.role_labels).map(([role,label])=>`<option value="${escapeHTML(role)}">${escapeHTML(label)} (${summary.role_counts[role]||0})</option>`).join('');
  $('search').value=''; $('cluster-filter').value=''; $('hops').value='1';
  const currentRun=runId;
  const clusters=await api(`/api/runs/${currentRun}/clusters`);
  if (currentRun!==runId) return;
  $('cluster-filter').innerHTML='<option value="">Все сообщества</option>'+clusters.sort((a,b)=>b.n_nodes-a.n_nodes).map(c=>`<option value="${c.cluster_id}">№${c.cluster_id+1} · ${c.n_nodes} счетов</option>`).join('');
  await loadList(true);
}
...
async function loadList(selectFirst=false) {
  ...
  $('node-list').innerHTML=data.items.length?data.items.map(n=>`<button class="node-row${n.gid===selectedId?' selected':''}" data-gid="${escapeHTML(n.gid)}" title="${escapeHTML(n.gid)}"><span class="rank">${n.rank}</span><span class="row-content"><span class="row-id">…${escapeHTML(n.gid.slice(-10))}</span><span class="row-role"><i class="role-dot" style="background:${colors[n.role]}"></i>${escapeHTML(n.role_label)}</span></span><span class="row-score">${Math.round(n.priority_score*100)}</span></button>`).join(''):'<div class="empty">Счета не найдены.<br>Измените gid или фильтры.</div>';
...
function renderDetail(detail) {
  const n=detail.node;
  const score=Math.round(n.priority_score*100);
  const contributions={structure:'Структурная значимость',seed_branches:'Схождение стартовых ветвей',observed_flow:'Наблюдаемый оборот',role_signals:'Выраженность роли'};
...
function showQuality(){
  if(!summary)return;
  showInfo('Качество и границы данных',`<div class="info-metric"><span>Набор</span><strong>${escapeHTML(summary.label)}</strong></div><div class="info-metric"><span>Слабосвязные компоненты с изолятами</span><strong>${summary.weak_components}</strong></div><div class="info-metric"><span>Сообщества Louvain</span><strong>${summary.cluster_count}</strong></div><div class="info-metric"><span>Изолированные счета</span><strong>${summary.isolate_count}</strong></div>${summary.warnings.map(w=>`<div class="quality-item">${escapeHTML(w)}</div>`).join('')}<p class="muted">Проверены обязательные поля, идентификаторы, ссылки между таблицами, суммы в тиынах и число операций. Гипотезы не имеют истинных меток для оценки accuracy.</p>`);
}
