...
const featureLabels={structure:'Положение среди связей',seed_branches:'Связь со стартовыми счетами',observed_flow:'Объём переводов',role_signals:'Признаки роли'};
...
function attachInvestigationActions(){
  const jump=document.createElement('button');jump.className='button primary analysis-jump';jump.textContent='Проверить переводы этого счёта ↓';jump.id='investigation-jump';
  ...
  $('node-detail').querySelector('.case-reason').after(jump);
...
function renderPaths(data){
  $('investigation-body').innerHTML=`<p class="investigation-note">Выберите маршрут слева. Справа показаны счета и операции, которыми можно проверить последовательность переводов.</p><details class="search-scope"><summary>Какие маршруты показаны: ${data.shown} из ${data.candidate_count}</summary>${escapeHTML(data.search_note)}</details>${data.paths.length?`<div class="path-layout"><div class="path-list">${data.paths.map((p,i)=>`<button class="path-option" data-path-index="${i}"><strong>${p.direction==='upstream'?'От стартового счёта':'От стартового счёта дальше'} · ${p.edges.length} шаг.</strong><span class="path-status ${p.status}">${escapeHTML(p.status_label)}</span><small>…${escapeHTML(p.gids[0].slice(-8))} → …${escapeHTML(p.gids.at(-1).slice(-8))}</small></button>`).join('')}</div><div class="path-detail" id="path-detail"></div></div>`:'<div class="empty">Цепочки в пределах поиска не найдены. Можно проверить операции и полноту выгрузки.</div>'}<p class="investigation-note" style="margin-top:20px">${escapeHTML(data.temporal_note)}</p>`;
